...
## Build and Run
- Build: `dotnet build`
- Run: `dotnet run` — starts Kestrel on https://localhost:13888 / http://localhost:13889 (dev `launchSettings.json`)
- Deploy: Docker Compose, built from source on the target LXC (see `INSTALL.md` at repo root) — `Dockerfile` + `docker-compose.yml` live at the repo root, not under `PeopleManager/`. Update flow is `git pull && docker compose up -d --build`; the SQLite DB lives in the `peoplemanager-data` named volume and survives rebuilds. `Program.cs` runs EF Core migrations automatically on startup.
